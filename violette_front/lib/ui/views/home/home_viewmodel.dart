...
void _listenToPendingRequests(String artistId) {
  ...
  _pendingRequestsSub =
      _bookingService.getPendingRequestsForArtist(artistId).listen((bookings) async {
...
Future<void> _loadShowDatesForRequests() async {
  for (final booking in pendingRequests) {
    ...
    try {
      ...
      final showDate = await _showDateService.getShowDateStream(dateId).first;
  ...
  final validIds = pendingRequests
      .map((b) => b.dateId)
      .whereType<String>()
      .toSet();
...
/// - accept == false => pendingConfirmation -> refused (et libère une place côté gérant)
  Future<void> respondToRequest(ArtistBooking booking, bool accept) async {
    ...
    try {
      await _bookingService.respondToRequest(dateId, booking.artistId, accept);
      
      // Feedback
      _snackbarService.showSnackbar(
        message: accept ? "C'est noté ! Présence confirmée." : "C'est noté ! Date refusée.",
        duration: const Duration(seconds: 2),
      );

      // Optimistic Update: Remove from list immediately
      pendingRequests.removeWhere((b) => b.artistId == booking.artistId && b.dateId == dateId);
      rebuildUi();
