...
| **DATE-REC-06** | Gérant connecté. Date existante de sa compagnie, créée avec l'ensemble de sa feuille de route (adresse complète, contact client, description). | 1. Ouvrir le planning gérant. 2. Sélectionner une date pour déplier son détail. 3. Appuyer sur le bouton « Détail ». | La fiche de consultation plein écran s'ouvre et restitue l'intégralité des informations de la date : heure de convocation, **adresse complète non tronquée**, contact client (nom et téléphone), description, effectif, ainsi que le statut de la date. La liste des artistes y figure en **lecture seule** : nom, engagement et disponibilité visibles, sans case à cocher ni bouton d'action (les actions restent dans le détail inline du planning). | Conforme au résultat attendu : la fiche affiche l'intégralité de la feuille de route, l'adresse n'est plus tronquée, et la liste des artistes est bien en lecture seule. | OK | |
...
| **A11Y-REC-04** | Gérant connecté. TalkBack actif. Planning gérant avec une date dépliée. | 1. Balayer le détail inline jusqu'au bouton « Détail ». 2. L'activer (double tap). 3. Parcourir la fiche de consultation au lecteur d'écran. | Le bouton « Détail » est un **arrêt de focus distinct**, annoncé comme bouton avec un libellé explicite, et activable au double tap (WCAG 2.1.1). Dans la fiche : le titre « Informations » est annoncé comme en-tête ; chaque ligne d'information est restituée d'un seul tenant (« libellé : valeur ») ; le statut de la date est annoncé ; les lignes artiste sont annoncées sans état de sélection (elles ne sont plus interactives). | Conforme au résultat attendu après correction (cf. BOGUE-05) : le bouton est focalisable, annoncé et activable ; la fiche est intégralement restituée au lecteur d'écran. | OK | BOGUE-05 |
| **A11Y-REC-05** | — (vérification de contraste sur l'ensemble des parcours). | Contrôler le contraste du texte de toutes les pastilles de statut (date, engagement, disponibilité), sur les parcours artiste et gérant. | Le texte de chaque pastille respecte le ratio AA de 4,5:1 sur son fond : la couleur du texte est dérivée de la luminance du fond (`contrastTextForStatusPill`), sans couleur codée en dur. Un même statut affiché à deux endroits présente le même rendu. | Conforme au résultat attendu après correction (cf. BOGUE-06) : ratios recalculés sur l'ensemble des couleurs de statut, toutes les pastilles atteignent au moins 4,5:1. | OK | BOGUE-06 |
...
| Accessibilité (WCAG 2.2 AA)                                    | Famille 6 (A11Y-REC-01 à 05)                          |
| Consultation de la fiche complète d'une date                   | Famille 2 (DATE-REC-06) ; Famille 6 (A11Y-REC-04)     |
